...
def loki_query(group_id, time_range, namespace="vcluster--usdf-prompt-processing", 
               container="lsstcam"):
    ...
    all_phrases = [START_PHRASE, FINISH_PHRASE, "Running '/app/"] + list(MIDDLE_PHRASES.values())
    search_pattern = "|".join(all_phrases)
    logql = f'{{namespace="{namespace}", container="{container}"}} |~ "{search_pattern}"'
            ...
            for value in stream.get('values', []):
                ...
                try:
                    ...
                    if detector is None and 'detector=' in message:
                        try:
                            detector_str = message.split('detector=')[1].split(',')[0].split(')')[0]
...
def count_detectors_from_logs(butler, visit_id, namespace="vcluster--usdf-prompt-processing",
                              container="lsstcam"):
    ...
    try:
        exp_record = list(butler.query_dimension_records(
    ...
    detector_checkpoint_times = {name: {} for name in MIDDLE_PHRASES.keys()}  # checkpoint -> {detector -> asctime}
    ...
    if reference == "exposure_end":
        ...
        if exposure_end_timestamp is None:
            print("Warning: exposure_end requested but timespan.end not available, falling back to start")
    ...
    for detector in detector_start_times:
        start_ts = _parse_asctime(detector_start_times[detector])
        ref_ts = get_reference_time(detector) if reference_timestamp is None else reference_timestamp
    ...
    for checkpoint_name in MIDDLE_PHRASES.keys():
        for detector in detector_checkpoint_times[checkpoint_name]:
            checkpoint_ts = _parse_asctime(detector_checkpoint_times[checkpoint_name][detector])
            ref_ts = get_reference_time(detector) if reference_timestamp is None else reference_timestamp
    ...
    stats = {
        ...
        'max_duration_reported': max(durations_reported) if durations_reported else None,
        'min_duration_reported': min(durations_reported) if durations_reported else None,
        'avg_duration_reported': sum(durations_reported) / len(durations_reported) if durations_reported else None,
        ...
        'avg_duration_actual': sum(durations_actual) / len(durations_actual) if durations_actual else None,
        'results': results,
        # Relative times for start
        'start_count': len(relative_times_start),
        'max_relative_time_start': max(relative_times_start) if relative_times_start else None,
        'min_relative_time_start': min(relative_times_start) if relative_times_start else None,
        'avg_relative_time_start': sum(relative_times_start) / len(relative_times_start) if relative_times_start else None,
        # Relative times for finish
        'finish_relative_count': len(relative_times_finish),
        'max_relative_time_finish': max(relative_times_finish) if relative_times_finish else None,
        'min_relative_time_finish': min(relative_times_finish) if relative_times_finish else None,
        'avg_relative_time_finish': sum(relative_times_finish) / len(relative_times_finish) if relative_times_finish else None,
    }
    ...
    for checkpoint_name in MIDDLE_PHRASES.keys():
        ...
        stats[f'max_relative_time_{checkpoint_name}'] = max(checkpoint_relative_times) if checkpoint_relative_times else None
        stats[f'min_relative_time_{checkpoint_name}'] = min(checkpoint_relative_times) if checkpoint_relative_times else None
        stats[f'avg_relative_time_{checkpoint_name}'] = sum(checkpoint_relative_times) / len(checkpoint_relative_times) if checkpoint_relative_times else None
